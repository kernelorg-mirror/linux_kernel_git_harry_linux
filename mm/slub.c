			...
			gfp_t gfp, unsigned int alloc_flags)
{
	...
	/*
	 * Most of the obj_exts arrays are allocated from normal kmalloc.
	 * However, normal kmalloc caches cannot allocate obj_exts arrays
	 * from the same kmalloc type regardless of the allocation size.
	 */
	...
	if (IS_ENABLED(CONFIG_DEBUG_VM)) {
		...
		if (exts_slab) {
			/*
			 * The vector must be allocated from normal or
			 * KMALLOC_NO_OBJ_EXT kmalloc caches to avoid recursion.
			 */
